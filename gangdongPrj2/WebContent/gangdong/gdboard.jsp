...
<table class="table table-striped">		
	...
	<tr>
		<td>${g.gdnum }</td>
		<td> <a class="text-info" href="gdboardDetail.do?d=${g.gdnum }">${g.gdtitle }</a> </td>
		<td>${g.gdwriter }</td>
		<td>${g.gddate }</td>
		<td>${g.gdcnt }</td>
	</tr>
...
<a class="btn btn-info pull-right" href="gdboardReg.do">작성</a>
<br />
...
<c:if test="${start!=1 }">
	<a class="text-info" href="gdboard.do?pg=${start-5 }&f=${param.f==null?'gdtitle':param.f}&q=${query}">prev</a>
</c:if>
<c:forEach var="i" begin="${start }" end="${start+4 }">
	<c:if test="${i<=end }">
		<c:if test="${i==page }">
			<strong>
				<a class="text-info" href="gdboard.do?pg=${i }&f=${param.f==null?'gdtitle':param.f}&q=${query}">${i }</a>
			</strong>
		</c:if>
		<c:if test="${i!=page }">
			<a class="text-info" href="gdboard.do?pg=${i }&f=${param.f==null?'gdtitle':param.f }&q=${query}">${i }</a>
